...
```shell
docker-compose exec e2e python3 -m vedro run --md-ext [ by default ALL ]
```

```shell
docker-compose exec e2e python3 -m vedro run --md-ext=kafka
```

```shell
docker-compose exec e2e python3 -m vedro run --md-ext=kafka,postgres_db
```
